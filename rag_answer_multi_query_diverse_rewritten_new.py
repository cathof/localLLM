...
def build_factual_json_schema(catalog: ErrorCatalog) -> Dict[str, Any]:
    ...
    """
    main_labels   = sorted(catalog.allowed_main_labels)
    ...
    for subs in catalog.allowed_subclasses_by_main_label.values():
...
def build_factual_review_messages(
    ...
    system = (
        ...
        "  - Feldnamen wie 'type', 'description', 'reason', 'category', 'Fehler', 'Art', 'Beschreibung'\n\n"
        ...
        "  hauptklasse         (aus der Taxonomie oben)\n"
        "  subklasse           (aus der Taxonomie oben)\n"
        "  aenderungstyp       (aus der Taxonomie oben)\n"
        "  schweregrad         niedrig | mittel | hoch\n"
        "  stelle_im_segment   exakter Originalausschnitt aus dem Segment\n"
        "  begruendung         beginnt zwingend mit 'Laut [S1_R_1]:'\n"
        "  source_refs         Liste der verwendeten Quell-Refs\n\n"
        "Format — exakt so, keine Abweichungen:\n"
        "{\"errors\":[{"
        "\"hauptklasse\":\"Rechtskonformität\","
        "\"subklasse\":\"Strafrecht\","
        "\"aenderungstyp\":\"Normativer Mangel\","
        "\"schweregrad\":\"hoch\","
        "\"stelle_im_segment\":\"<kurzer Originalausschnitt>\","
        "\"begruendung\":\"Laut [S1_R_2]: <relevante Aussage aus der Quelle>\","
        "\"source_refs\":[\"S1_R_2\"]}]}"
    )
    user = (
        f"DOKUMENTSEGMENT:\n{segment_text.strip()}\n\n"
...
def normalize_factual_errors(raw_errors: List[Any], catalog: ErrorCatalog) -> List[Dict[str, Any]]:
    ...
    for item in raw_errors:
        ...
        if not source_refs:
            print(f"[DROP] no source_refs — stelle={stelle[:60]!r}")
            continue
...
_LT_ALLOWED_RULE_PREFIXES = (
    "DE_CASE",              # capitalisation errors (Beschuldigte → beschuldigte)
    "GERMAN_SPELLER",       # pure spelling errors
    "MORFOLOGIK",           # morphological spelling
    "AGREEMENT",            # article-noun-adjective agreement
    "COMPOUND",             # compound word errors
    "UPPERCASE_SENTENCE",   # sentence starting with lowercase
    "COMMA_PARENTHESIS",    # missing comma around parenthetical
    "DE_DOUBLE_PUNCTUATION", # double punctuation
)
...
def _run_language_tool(segment_text: str, catalog: ErrorCatalog) -> List[Dict[str, Any]]:
    ...
    for m in matches:
        ...
        length = int(_lt_attr(m, "errorLength", "length", default=0))
        ...
        if isinstance(replacements_raw, list):
            replacements = [str(r) for r in replacements_raw]
        else:
            replacements = []
...
def _run_spacy_adjective_check(segment_text: str) -> List[Dict[str, Any]]:
    ...
    Rule: if a token is tagged ADJ with dependency nk (noun kernel modifier),
    its syntactic head is a NOUN, it starts with an uppercase letter, and
    the immediately preceding token is a DET — then it is an attributive
    adjective and should be lowercase.

    Validated test results:
      DETECTS:  'Die Beschuldigte Person', 'Der Beschuldigte Fahrer'
      IGNORES:  'Die beschuldigte Person' (correct), 'Die Beschuldigte' (nominalised noun)
      MISSES:   compound forms like 'Unfallbeteiligten' (parsed as NOUN by model)

    Ref: German TIGER treebank label 'nk' = noun kernel dependent.
    """
    nlp = _get_spacy_nlp()
    if nlp is None:
        return []
    ...
    try:
        ...
        print(f"[WARN] spaCy parse failed: {e}")
        return []

    # ← add these 3 lines temporarily
    for token in doc:
        if token.text[0].isupper() and token.pos_ == "ADJ":
            print(f"[DEBUG-SPACY] ADJ={token.text!r} dep={token.dep_} head={token.head.text!r}({token.head.pos_}) prev={doc[token.i-1].pos_ if token.i > 0 else 'START'}")
        ...
        if (
            ...
            findings.append({
                ...
                "begruendung": (
                    ...
                    f"'{token.text}' → '{correct}' (Kontext: '{context}')"
                ),
                "vorschlag": correct,
            })
...
def filter_language_findings_by_plausibility(
    ...
    for item in findings:
        ...
        # spaCy adjective capitalisation findings are special:
        # the correction may differ only by case, and that is exactly the error.
        is_spacy_adj = "attributives adjektiv" in begruendung
        ...
        # Schweizer ss/ß:
        # Skip equivalent ß/ss suggestions, but do NOT skip spaCy case-only fixes.
        if not is_spacy_adj and _normalize_eszett(stelle) == _normalize_eszett(vorschlag):
            continue

        # keine Eigennamen-/Bezeichnungs-Normalisierung
        if not is_spacy_adj and _looks_like_named_entity(stelle):
            continue
        ...
        if len(stelle.split()) <= 3 and len(vorschlag.split()) <= 3:
            if stelle.lower() != vorschlag.lower():
                if not is_spacy_adj and not any(x in begruendung for x in [
                    "orthograf",
                    "rechtschreib",
                    "grammatik",
                    "zeichensetzung",
    ...
    for item in findings:
        ...
        # spaCy adjective capitalisation findings are exempt from the
        # named-entity and short-phrase filters because the capitalised
        # token IS the error — filtering it as a named entity is wrong.
        is_spacy_adj = "attributives adjektiv" in begruendung
        ...
        if len(stelle.split()) <= 3 and len(vorschlag.split()) <= 3:
            if stelle.lower() != vorschlag.lower():
                # spaCy adjective findings and clear grammar terms always pass
                if not is_spacy_adj and not any(x in begruendung for x in [
                    "orthograf", "rechtschreib", "grammatik",
                    "zeichensetzung", "komma", "adjektiv"]):
                    continue
